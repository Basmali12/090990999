...
export function periodDay(date:string,branch:string,currency:string){
 const [y,m,d]=date.split('-').map(Number);const seed=periodCurrencies.includes(currency)?(y*3+m*7+d*11+periodBranches.indexOf(branch)*13+periodCurrencies.indexOf(currency)*5)%31+5:0;const scale=currency==='IQD'?1000:1;
 ...
 const inside=receipt+buy,inOutside=payment+sell,net=inside-inOutside,opening=periodCurrencies.includes(currency)?(50000+(m*100+d*20))*scale:0;
